...
import e_MANADATE_CONFIG from './e-manadate.Config';
import { response } from 'express';
import { v4 as uuid } from 'uuid';

const prisma = new PrismaClient();
const {e_manadate_key_id,e_manadate_key_secret}=e_MANADATE_CONFIG
...
async function getPanDetails(pan: string) {
  try {
    const response = await axios.post(e_manadate_key_id, pan, {
      headers: {
        Authorization: `Bearer ${e_manadate_key_secret}`,
